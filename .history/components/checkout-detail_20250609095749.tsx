...
const ReserveForm = ({room}: {room:RoomDetailProps}) => {
  ...
  // Create a server action wrapper that includes current dates
  const reserveAction = async (prevState: any, formData: FormData) => {
    return createReserve(room.id, room.price, startDate, endDate, prevState, formData)
  }

  const [state, formAction, isPending] = useActionState(reserveAction, null)
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-gray-500">
              Rp {roomPrice.toLocaleString('id-ID')} per night
            </div>
          </div>
        </div>
      </div>
          ...
          >
            {isPending ? (
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                Reserve Now - Rp {totalPrice.toLocaleString('id-ID')}
              </div>
            )}
          </button>
        </div>
