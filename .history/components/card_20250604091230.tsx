...
//           <Link
//             href={`/room/${room.id}`}
//             className="px-6 py-2.5 font-semibold text-white bg-teal-500 rounded-full hover:bg-teal-600 transition-all duration-200 hover:scale-105 hover:shadow-lg text-sm"
//           >
//             Pesan Sekarang
//           </Link>
//         </div>
...
const Card = ({room}: {room: Room}) => {
  ...
  console.log("Link URL:", `/room/${room.id}`);
    ...
    <div className="bg-white shadow-lg rounded-2xl transition-all duration-300 hover:shadow-xl hover:-translate-y-2 overflow-hidden group">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          {/* DEBUG: Tambah onClick handler untuk test */}
          <Link
            href={`/room/${room.id}`}
            onClick={(e) => {
              console.log("Link clicked!");
              console.log("Navigating to:", `/room/${room.id}`);
              // Uncomment baris di bawah jika ingin test manual navigation
              // e.preventDefault();
            }}
            className="px-6 py-2.5 font-semibold text-white bg-teal-500 rounded-full hover:bg-teal-600 transition-all duration-200 hover:scale-105 hover:shadow-lg text-sm"
          >
